...
public final class Router: Presentable {
    ...
    public func presentCard(_ module: Presentable?, isDraggable: Bool = true, isDismissOnDimEnabled: Bool = true, isCloseable: Bool = false) {
        guard let rootController = rootController, let source = rootController.topViewController, var destination = module?.toPresent() else { return }
        if isCloseable {
            destination = rootController.embedInCloseableController(destination)
        }
        let segue = CardSegue(source: source, destination: destination, isDraggable: isDraggable, isDismissOnDimEnabled: isDismissOnDimEnabled)
        segue.perform()
     }
    
    public func presentActionSheet(_ module: Presentable?, interactive: Bool = false) {
        guard let destination = module?.toPresent(), let source = rootController?.topViewController else { return }
        let segue = ActionSheetSegue(identifier: nil, source: source, destination: destination, interactive: interactive)
        segue.perform()
    }
